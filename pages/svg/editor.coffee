...
class Joint extends Shape
  constructor: (@parent, x = 0, y = 0, cpX = 0, cpY = 0) ->
    ...
    @el.on "dragmove", (e) =>
      ...
      tmpline.plot @lineTo { x: pos.x, y: pos.y }
  ...
  lineTo: (p) =>
    cp = { x: @cpX(), y: @cpY() }
    # "M #{@x()},#{@y()} C #{cp.x},#{cp.y} #{p.x},#{p.y}  #{p.x},#{p.y}"
    "M #{@x()},#{@y()} L #{p.x},#{p.y}"

  x: -> new SVG.Point(@el.cx(), @el.cy()).transform(@parent.el.matrixify()).x
  y: -> new SVG.Point(@el.cx(), @el.cy()).transform(@parent.el.matrixify()).y

  cpX: -> new SVG.Point(@cp.cx(), @cp.cy()).transform(@parent.el.matrixify()).x
  cpY: -> new SVG.Point(@cp.cx(), @cp.cy()).transform(@parent.el.matrixify()).y
...
class Connection extends Shape
  ...
  line: =>
    "M #{@joint1.x()},#{@joint1.y()} L #{@joint2.x()},#{@joint2.y()}"
  curve: =>
    cp1 = {x:@joint1.cpX(), y:@joint1.cpY()}
    cp2 = {x:@joint2.cpX(), y:@joint2.cpY()}
...
class Editor
  constructor: (name) ->
    ...
    @draw = SVG(name).size('100%', '100%').viewbox(0,0,1000,1000/1.7)
    @sample()
    @init()
  ...
  sample: ->
    ...
    # @draw.circle(20).fill('red').center(320, 320)
...
class Surface3
  ...
  update: =>
    ...
    @cline2.plot @cp3.cx(), @cp3.cy(), @cp4.cx(), @cp4.cy()
